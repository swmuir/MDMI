...
import org.openhealthtools.mdht.mdmi.engine.XDataChoice;
...
/**
 * Implementation of the IExpressionInterpreter for NRL.
 * It is used by the runtime to evaluate NRL expressions at runtime.
 *
 * @author goancea
 */
public class NrlAdapter implements IRuntimeModelAdapter, IExpressionInterpreter {
    ...
    private void addChildValue( XValue parent, XValue child, int index ) {
        if( parent.getDatatype().isSimple() ) {
            ...
            if( x == null ) {
                x = new XDataStruct(parent, (DTCStructured)parent.getDatatype());
                parent.setValue(x);
            }
            x.setValue(child);
        }
            ...
            if( x == null ) {
                x = new XDataChoice(parent, (DTCChoice)parent.getDatatype());
                parent.setValue(x);
            }
            x.setValue(child);
        }
    ...
    public void removeChild( Object parent, Object child ) throws ExecutionException {
        ...
        if( trg.getDatatype().isSimple() ) {
            ...
            if( x == null ) {
                x = new XDataStruct(trg, (DTCStructured)trg.getDatatype());
                trg.setValue(x);
            }
            ...
            if( x == null ) {
                x = new XDataChoice(trg, (DTCChoice)trg.getDatatype());
                trg.setValue(x);
            }
            x.clearValue();
        }
        ...
        if( parent instanceof XElementValue ) {
            ...
            if( child instanceof XElementValue ) {
                ...
                if( v.getDatatype().isSimple() )
                    ...
                    if( xdt == null ) {
                        xdt = new XDataStruct(v, (DTCStructured)v.getDatatype());
                        v.setValue(xdt);
                    }
                    int pos = xdt.setValue(xchild);
                    if( pos < 0 )
                    ...
                    if( xdc == null ) {
                        xdc = new XDataChoice(v, (DTCChoice)v.getDatatype());
                        v.setValue(xdc);
                    }
                    xdc.setValue(xchild);
                }
            ...
            if( child instanceof XValue ) {
                ...
                if( xparent.getDatatype().isSimple() )
                    ...
                    if( xdt == null ) {
                        xdt = new XDataStruct(xparent, (DTCStructured)xparent.getDatatype());
                    ...
                    if( xdc == null ) {
                        xdc = new XDataChoice(xparent, (DTCChoice)xparent.getDatatype());
    ...
    private ArrayList<Object> getChildren( Object parent, String name, boolean breakAtFirst ) {
        ...
        if( parent instanceof XElementValue ) {
            ...
            if( name == null || SemanticElement.VALUE_NAME.equalsIgnoreCase(name) )
                ...
                a.addAll(getXValueChildren(xe.getValue(), name, breakAtFirst));
            }
        }
        else if( parent instanceof XValue ) {
    ...
    private ArrayList<Object> getXValueChildren( XValue parent, String name, boolean breakAtFirst ) {
        ...
        if( xv.getDatatype().isStruct() ) {
            ...
            for( int i = 0; i < xvs.size(); i++ ) {
                ...
                ArrayList<XValue> vs = xs.getValues();
