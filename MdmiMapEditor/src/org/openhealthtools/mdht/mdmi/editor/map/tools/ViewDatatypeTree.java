...
public class ViewDatatypeTree extends PrintableView {
	...
	private DatatypeTree   m_tree;
	
	public ViewDatatypeTree(MdmiDatatype datatype) {
		// View of Datatype 'foo'
		...
		// create tree
		m_tree = new DatatypeTree(datatype);
		setCenterComponent(new JScrollPane(m_tree));
		
		// expand all
		m_tree.expandAllComplexTypes(m_tree.getRoot());
		
		initFrame();
	}
	
	public ViewDatatypeTree(Collection<MdmiDatatype> datatypes) {
		// View Datatypes
		setTitle( s_res.getString("ViewDatatypeTree.viewDatatypes") );

		// create tree
		m_tree = new DatatypeTree(datatypes);
		setCenterComponent(new JScrollPane(m_tree));
